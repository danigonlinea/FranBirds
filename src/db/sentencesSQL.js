export default {
  ...
  getBirdByGlobalId:
    "SELECT a.globalId, a.id, a.type, a.gender, a.fatherId AS 'fatherIdGlobal', a.motherId AS 'motherIdGlobal', a.notes, a.photo, (SELECT b.id FROM bird b WHERE b.globalId = (SELECT d.fatherId FROM bird d WHERE d.globalId = ?)) AS 'fatherId', (SELECT c.id FROM bird c WHERE c.globalId = (SELECT e.motherId FROM bird e WHERE e.globalId = ?)) AS 'motherId', a.fk_jail FROM bird a WHERE a.globalId = ?",
  ...
  insertBird:
    'INSERT INTO bird (id, type, gender, fatherId, motherId, notes, photo, fk_jail) values (?, ?, ?, ?, ?, ?, ?, ?)',
  updateBird:
    'UPDATE bird SET id = ?, type = ?, gender = ?, fatherId = ?, motherId = ?, notes = ?, photo = ?, fk_jail = ? WHERE globalId = ?',
